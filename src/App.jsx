...
// Altura visual por franja 30 min (uniforme)
const ROW_PX = 72;
const SLOT_ROW_CLASS = "h-[72px]";
...
async function fetchBookingsForDay(day) {
  ...
  if (error) { console.error(error); toast.error("No pude cargar reservas"); return []; }
  return data || [];
}
async function upsertBooking(b) {
  const payload = {
    id: b.id, room: b.room, person: b.person, purpose: b.purpose,
...
export default function App() {
  ...
  const [settings, setSettings] = useState({
    ...
    startHour: 8, endHour: 22, requireName: true, allowPast: false
  });
  const [openSettings, setOpenSettings] = useState(false);

  useEffect(() => { (async () => setBookings(await fetchBookingsForDay(currentDay)))(); }, [currentDay]);
  ...
  const slots = useMemo(() => enumerateSlots(currentDay, settings.startHour, settings.endHour),
  ...
  async function addOrUpdateBooking(newB) {
    ...
    const s = fromISO(newB.start), e = fromISO(newB.end);
    if (!settings.allowPast && isBefore(e, new Date())) { toast.error("No se permiten reservas en el pasado"); return false; }
    const conflict = dayBookings.some(b => b.id !== newB.id && b.room === newB.room &&
    ...
    try { await upsertBooking(newB); toast.success("Reserva guardada"); setBookings(await fetchBookingsForDay(currentDay)); return true; }
    catch (err) { console.error(err); toast.error("No se pudo guardar"); return false; }
  }
  async function deleteBooking(id) {
    try { await deleteBookingDb(id); toast("Reserva eliminada"); setBookings(await fetchBookingsForDay(currentDay)); }
    catch (e) { console.error(e); toast.error("No se pudo eliminar"); }
  }
  function clearAll() { toast("Para vaciar todo, hazlo desde la base de datos."); }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4">
      ...
      <header className="max-w-6xl mx-auto mb-4 flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={clearAll} className="px-3 py-2 rounded-lg border bg-white hover:bg-slate-50 text-sm flex items-center gap-2">
              <Download className="w-4 h-4" /> Vaciar
            </button>
            <button onClick={() => setOpenSettings(true)} className="px-3 py-2 rounded-lg border bg-white hover:bg-slate-50 text-sm flex items-center gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input type="date" className="ml-2 px-3 py-2 border rounded-lg text-sm bg-white"
                 value={toDateInput(currentDay)} onChange={(e) => setCurrentDay(fromDateInput(e.target.value))} />
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white border rounded-2xl shadow-sm">
            <div className="p-4 border-b flex items-center gap-2 font-semibold text-slate-700">
              <CalendarIcon className="w-5 h-5" /> Nueva reserva
            </div>
            <div className="p-4">
              <BookingForm key={format(currentDay, "yyyy-MM-dd")} currentDay={currentDay} settings={settings} onSubmit={addOrUpdateBooking} />
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white border rounded-2xl shadow-sm">
            <div className="p-4 border-b flex items-center gap-2 text-sm text-slate-600">
              <Clock className="h-4 w-4" /> Tramos de {SLOT_MINUTES} min · {String(settings.startHour).padStart(2, "0")}:00–{String(settings.endHour).padStart(2, "0")}:00
            </div>
            <div className="p-2">
              <DayGrid day={currentDay} slots={slots} rooms={settings.rooms} bookings={dayBookings}
                       onEdit={(b) => { if (!window.confirm("¿Editar esta reserva?")) return; window.dispatchEvent(new CustomEvent("load-booking", { detail: b })); }}
      ...
      <footer className="max-w-6xl mx-auto mt-6 text-center text-xs text-slate-500">
        Hecho con ❤️ · Supabase realtime · columnas con separadores
      </footer>
    </div>
  );
}
...
function WeekStrip({ currentDay, onSelect }) {
  ...
  return (
    <div className="flex gap-2 overflow-x-auto pb-1">
      <button onClick={() => onSelect(addDays(currentDay, -7))} className="px-2 py-1 border rounded-lg bg-white hover:bg-slate-50 text-xs flex items-center gap-1">
        <ChevronLeft className="w-4 h-4" /> Semana -
      </button>
      {days.map((d) => {
        const isActive = format(d, "yyyy-MM-dd") === format(currentDay, "yyyy-MM-dd");
        return (
          <button key={d.toISOString()} onClick={() => onSelect(startOfDay(d))}
...
function MiniCalendar({ selected, onSelect }) {
  ...
  function isSameDay(a, b) { return format(a, "yyyy-MM-dd") === format(b, "yyyy-MM-dd"); }

  return (
    <div className="bg-white border rounded-2xl shadow-sm">
      <div className="p-3 border-b flex items-center justify-between">
        <button className="p-2 rounded-lg border hover:bg-slate-50" onClick={() => setViewMonth(subMonths(viewMonth, 1))}><ChevronLeft className="w-4 h-4" /></button>
        <div className="font-semibold">{format(viewMonth, "MMMM yyyy", { locale: es })}</div>
        <button className="p-2 rounded-lg border hover:bg-slate-50" onClick={() => setViewMonth(addMonths(viewMonth, 1))}><ChevronRight className="w-4 h-4" /></button>
      </div>
...
function BookingForm({ currentDay, settings, onSubmit }) {
  ...
  useEffect(() => {
    function handleLoad(e) {
      const b = e.detail;
      setId(b.id); setRoom(b.room);
      setPerson(b.person || ""); setPurpose(b.purpose || "");
  ...
  const timeOptions = useMemo(() => {
    ...
    for (let h = settings.startHour; h < settings.endHour; h++) {
      for (let m = 0; m < 60; m += SLOT_MINUTES) arr.push(`${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}`);
    }
    return arr;
  ...
  async function handleSubmit(e) {
    ...
    if (ok) { setId(null); setPurpose(""); toast.success("Lista para una nueva reserva"); }
  }
  ...
  function exportICS() {
    ...
    a.href = url; a.download = `${format(start, "yyyyMMdd-HHmm")}-${roomName.replace(/\s+/g, "_")}.ics`; a.click();
    ...
    <form id="bookingForm" onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={exportICS} className="px-3 py-2 rounded-lg border bg-white hover:bg-slate-50 text-sm flex items-center gap-2">
          <Download className="h-4 h-4" /> .ics
        </button>
      </div>
  ...
  return (
    <div className="overflow-x-auto">
      ...
      >
        ...
        {rooms.map((r, i) => (
          <div key={i} className="px-2 py-2 text-sm font-semibold text-slate-700 border-b border-l border-slate-200">{r}</div>
        ))}

        {/* Filas */}
        {slots.map((t, rowIdx) => (
          <React.Fragment key={rowIdx}>
            <div className={`px-2 py-3 text-xs text-slate-500 border-b border-slate-100 sticky left-0 bg-white z-10 ${SLOT_ROW_CLASS}`}>
              {timeToLabel(t)}
            </div>
            {rooms.map((_, colIdx) => (
...
function Cell({ time, roomIndex, bookings, onEdit, onDelete }) {
  ...
  // Separador vertical por columna
  const colSeparator = "border-l border-slate-100";

  // Sin reserva
  if (!active) {
    return <div className={`${colSeparator} border-b ${SLOT_ROW_CLASS} p-1 overflow-hidden hover:bg-slate-50 transition-colors`}></div>;
  }

  const isStart = isEqual(fromISO(active.start), time);

  // Continuación: sin fondo ni bordes para que la tarjeta del inicio ocupe visualmente todo
  if (!isStart) {
    return <div className={`${colSeparator} ${SLOT_ROW_CLASS} p-1 overflow-hidden`}></div>;
  }
  ...
  return (
    <div className={`${colSeparator} ${SLOT_ROW_CLASS} p-1 overflow-hidden`}>
      <div
        className="h-full p-2 rounded-2xl shadow-sm bg-white border border-slate-200 flex flex-col gap-1"
        style={{ height: `${rows * ROW_PX - 8}px` }} // -8 por padding vertical del wrapper
      >
...
function SettingsPanel({ settings, setSettings, onClose }) {
  ...
  function save() { setSettings(local); toast.success("Ajustes guardados"); onClose?.(); }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {local.rooms.map((r, i) => (
            <input key={i} value={r} onChange={(e) => {
              const copy = [...local.rooms]; copy[i] = e.target.value; setLocal({ ...local, rooms: copy });
...
function generateICS({ title, description, location, start, end }) {
  const uid = uuidv4();
  const lines = [
    "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//ReservaDespachos//v1//ES","CALSCALE:GREGORIAN","METHOD:PUBLISH","BEGIN:VEVENT",
    `UID:${uid}`,`DTSTAMP:${toICSDate(new Date())}`,`DTSTART:${toICSDate(new Date(start))}`,`DTEND:${toICSDate(new Date(end))}`,
    `SUMMARY:${escapeICS(title)}`,`DESCRIPTION:${escapeICS(description)}`,`LOCATION:${escapeICS(location)}`,
